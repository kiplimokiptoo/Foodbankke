# FoodBankKe 🇰🇪
FoodBankKe is a modern, responsive web application built for a Kenyan NGO dedicated to distributing meals and clean water to vulnerable families. The project uses a professional "Water and Food" aesthetic with an Emerald and Blue color palette.

## 🚀 Live Demo
The project is officially deployed and can be viewed here:
Open FoodBankKe on Netlify
[Click here to view the live site](https://resplendent-salmiakki-683513.netlify.app/)

## 🛠️ Project Features
### Four-Page Navigation: 
Includes Home, Services, Contributions, and Contact Us.

### Modern UI: 
Built with Tailwind CSS for a mobile-first, sleek design.

### Donor Appreciation: 
Integrated "Thank You" sections to acknowledge supporters.

## Direct Contact Tools:

### WhatsApp Chat: 
Instant message button for quick inquiries.

### Click-to-Call: 
Interactive phone link for mobile users (0708319101).

### Click-to-Email: 
Automated mail link for official correspondence.

## 👥 Collaborators
This project is a result of the collective effort of:

Emmanuel

Precious

Kiplimo

Obinda

## 💻 Development Workflow
### 1. Installation & Setup
To run this project locally, clone the repository using Git:

Bash
#### Clone the repository
git clone https://github.com/your-username/foodbankke.git

#### Navigate into the folder
cd foodbankke
### 2. How to Contribute
We use Git for version control. Please follow this workflow when making changes:

Pull the latest changes: git pull origin main

Make your edits to the HTML files.

Stage your changes: git add .

Commit your work: git commit -m "Added a new feature/fixed a bug"

Push to GitHub: git push origin main

Note: Netlify is linked to this repository and will automatically deploy your changes when you push to GitHub.

## 📖 Technical Stack
HTML5: Structure and content.

Tailwind CSS: Styling and layout via CDN.

Git & GitHub: Version control and collaboration.

Netlify: Continuous deployment and hosting.

## 📄 License
This project was created for community support and educational purposes.

## Future Improvements
Integration of a backend to handle the contact form submissions.

Live payment gateway for monetary donations.

Dynamic blog section for real-time project updates.
